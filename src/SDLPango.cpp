...
Surface::Surface(SDL_Renderer* r)
		...
		, mDefaultTextColor({0,0,0,255})
		, mBackgroundColor(ZERO_COLOR)
...
const std::string Surface::getText() {
	return mText;
}
...
TextWeight Surface::getDefaultTextWeight() {
...
TextAlignment Surface::getTextAlignment() {
...
TextAntialias Surface::getTextAntialias() {
...
SDL_Color Surface::getDefaultTextColor() {
...
SDL_Color Surface::getBackgroundColor() {
...
float Surface::getDefaultTextSize() {
...
std::string Surface::getDefaultTextFont() {
...
TextRenderer Surface::getTextRenderer() {
	...
	if (rendererName == nullptr) {
		p_log("Could not read Pango Cairo backend environment variable. Assuming native renderer.");
	...
	// p_log("Unknown Pango Cairo backend environment variable: " << rendererNameString << ". Assuming native renderer.");
	return TextRenderer::PLATFORM_NATIVE;
}
