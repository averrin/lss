...
public:
    ...
    bool isFullShadow()
    {
        return _shadows.size() == 1 && _shadows[0].start == 0 && _shadows[0].end == 1;
    }
};
    ...
    std::list<Vec> refresh(Vec pos, Vec bounds, Cells cells)
{
    ...
    result.sort([](const Vec a, const Vec b) { return a.y < b.y || a.y == b.y && a.x < b.x; });
    result.unique([](const Vec a, const Vec b) { return a.x == b.x && a.y == b.y; });
