...
typedef std::function<bool(std::vector<std::shared_ptr<Cell>>)>
    CheckTargetCallback;
class TargetEvent : public eb::Event {
public:
  TargetEvent(TargetCallback c, CheckTargetCallback ct)
      : eb::Event(nullptr), callback(c), checkTarget(ct) {}
  TargetEvent(std::shared_ptr<Cell> st, TargetCallback c,
              CheckTargetCallback ct)
